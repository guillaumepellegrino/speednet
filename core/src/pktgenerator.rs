...
pub fn udp_send(args: &ArgsClient, udp: UdpSocket) {
    println!("[PKTGEN.TX] UDP Send !");
    ...
    if let Err(e) = udp.set_write_timeout(Some(Duration::from_millis(1000))) {
        println!("Failed to set write timeout: {:?}", e);
        return;
    }
    ...
    loop {
        ...
        // TODO: use send_vectored() to improve performances ?
        //let mut buffers = Buffers::new(1492, 10);
        //let len = buffers.send(&udp);
        let len = match udp.send(buffer) {
            Ok(x) => x,
            Err(e) => {
                println!("[PKTGEN.TX] Write error: {:?}", e);
                break;
...
pub fn udp_recv(args: &ArgsClient, udp: UdpSocket, update: &Mutex<StreamResult>) {
    let bufferlen = args.get_bufferlen();
    let mut buffer = vec!(0; bufferlen as usize);
    let total_bytes_expected = args.get_total_bytes_expected();
    let mut pktcount = 0;
    ...
    println!("[PKTGEN.RX] UDP Recv !");

    if let Err(e) = udp.set_read_timeout(Some(Duration::from_millis(1000))) {
        println!("Failed to set read timeout: {:?}", e);
        return;
    }

    loop {
        let len = match udp.recv(&mut buffer) {
            Ok(x) => x,
            Err(e) => {
                println!("[PKTGEN.RX] Failed to read: {:?}", e);
                let mut update = update.lock().unwrap();
